...
void CBoard::ComputerThink()
{
	...
	int nowValue=Eval();
	int alpha=nowValue-10;
	int beta=nowValue+10;

	int value=AlphaBetaSearch(MaxDepth,alpha,beta);

	if(value>=beta) value=AlphaBetaSearch(MaxDepth,value,MaxValue);
	if(value<=alpha) value=AlphaBetaSearch(MaxDepth,-MaxValue,value);
